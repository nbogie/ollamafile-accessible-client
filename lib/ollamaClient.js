 ...
 * @param {AbortSignal} [signal] — when aborted, the upstream Ollama request is
 *   cancelled and the generator throws an AbortError. Callers should re-throw
 *   AbortError unwrapped so it can be distinguished from real connection errors.
...
export async function* streamChat(messages, signal) {
  ...
  try {
    ...
    // Aborts are a normal control-flow signal, not a failure mode — let them
    // through unwrapped so the server can distinguish "user pressed Stop"
    // from "Ollama is unreachable".
